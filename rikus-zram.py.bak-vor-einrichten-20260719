...
def aenderungen_sammeln(sys_, swp, einst, ziel_prozent, ziel_swappiness,
                        ziel_swap_gb=None, swap=None, platte=None):
...
def skript_bauen(schritte, sys_):
    ...
    for _text, shell, _sicherung in schritte:
        zeilen += shell
    # Den Turbo-Dienst NUR neu starten, wenn seine Einstellungen angefasst
    ...
    if any('zramswap' in z for _t, sh, _s in schritte for z in sh):
...
def bewerten(ram, zram, swap, swp, einst):
    ...
    if einst['vorhanden'] and not zram_laeuft:
        hinweise.append(t(
            f'zram ist in {sicher(einst["pfad"])} eingerichtet, läuft aber '
            'gerade NICHT. Meist hilft ein Neustart des Dienstes.',
            f'zram is configured in {sicher(einst["pfad"])} but is NOT running. '
            'Restarting the service usually helps.'))
